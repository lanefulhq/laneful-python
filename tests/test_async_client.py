...
class TestAsyncLanefulClient:
    ...
    async def test_send_email_success(self):
        ...
        response_data = {"status": "sent", "message_id": "msg_123"}

        with patch.object(
            self.client, "_make_request", return_value=response_data
        ) as mock_request:
            ...
            mock_request.assert_called_once_with(
                "POST", "/email/send", {"emails": [self.sample_email.to_dict()]}
            )

    @pytest.mark.asyncio
    async def test_send_email_with_mail_settings(self):
        """Mail settings are included on the async send payload."""
        response_data = {"status": "accepted", "message_ids": ["msg_123"]}
    ...
    async def test_send_emails_success(self):
        ...
        with patch.object(
            ...
            mock_request.assert_called_once_with(
                "POST",
                "/email/send",
                {"emails": [self.sample_email.to_dict(), self.sample_email.to_dict()]},
            )
    ...
    async def test_send_email_auth_error(self):
        ...
        with patch.object(
            self.client,
            "_make_request",
            side_effect=LanefulAuthError("Invalid authentication token"),
        ):
    ...
    async def test_send_email_connection_error(self):
        ...
        with patch.object(
            self.client,
            "_make_request",
            side_effect=LanefulError("Failed to connect to Laneful API"),
        ):
    ...
    async def test_send_email_timeout(self):
        ...
        with patch.object(
            self.client, "_make_request", side_effect=LanefulError("Request timed out")
        ):
            with pytest.raises(LanefulError) as exc_info:
                await self.client.send_email(self.sample_email)
    ...
    async def test_get_email_status(self):
        ...
        response_data = {"status": "delivered", "delivered_at": 1640995200}
    ...
    @pytest.mark.asyncio
    async def test_org_apis(self):
        """Async org methods parse responses and hit the same endpoints."""
        with patch.object(
            self.client,
            "_make_request",
            return_value={
                "unsubscribe_groups": [
                    {"unsubscribe_group_id": 9, "name": "Newsletters"}
                ]
            },
        ) as mock_request:
            groups = await self.client.list_unsubscribe_groups(42)
            assert groups.unsubscribe_groups[0].name == "Newsletters"
            mock_request.assert_called_once_with(
                "GET", "/workspaces/42/unsubscribe-groups", params=None
            )
        ...
        with patch.object(
            ...
            "_make_request",
            return_value={
                "unsubscribe_group": {"unsubscribe_group_id": 9, "name": "N"}
            },
        ) as mock_request:
            created = await self.client.create_unsubscribe_group(42, "N")
            assert created.unsubscribe_group_id == 9
            mock_request.assert_called_once_with(
                "POST", "/workspaces/42/unsubscribe-groups", {"name": "N"}
            )
        ...
        with patch.object(
            ...
            listing = await self.client.list_domains(
                42, ListDomainsParams(limit=25, filter_domain="example.com")
            )
            assert listing.domains[0].domain == "example.com"
            mock_request.assert_called_once_with(
                "GET",
                "/workspaces/42/domains",
                params=[("limit", "25"), ("filter[domain]", "example.com")],
            )

        with patch.object(
            self.client,
            "_make_request",
            return_value={"domain": {"domain": "example.com", "verified": True}},
        ) as mock_request:
            domain = await self.client.get_domain(42, "example.com")
            assert domain.verified is True
            await self.client.create_domain(
                42,
                CreateDomainRequest(
                    domain="example.com", tracking="t", return_path="rp"
                ),
            )
            await self.client.update_domain(42, "example.com", UpdateDomainRequest(""))
            await self.client.verify_domain(42, "example.com")

        with patch.object(
            self.client, "_make_request", return_value={"message": "deleted"}
        ) as mock_request:
            deleted = await self.client.delete_domain(42, "example.com")
            assert deleted.message == "deleted"
            mock_request.assert_called_once_with(
                "DELETE", "/workspaces/42/domains/example.com"
            )

        with patch.object(
            self.client, "_make_request", return_value={"radar": []}
        ) as mock_request:
            await self.client.list_domain_spam_ratio_radar(
                ListDomainSpamRatioRadarParams(workspace_ids=[1, 2])
            )
            mock_request.assert_called_once_with(
                "GET",
                "/analytics/radar/domain-spam-ratio",
                params=[("workspace_ids", "1"), ("workspace_ids", "2")],
            )
...
async def test_multiple_concurrent_requests():
    ...
    with patch.object(
        client, "_make_request", return_value=response_data
    ) as mock_request:
