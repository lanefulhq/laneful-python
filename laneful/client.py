...
from .models import Email, EmailResponse, EmailList, EmailResponseList
from .exceptions import LanefulError, LanefulAPIError, LanefulAuthError
...
class LanefulClient(BaseLanefulClient):
    ...
    def send_email(self, email: Email) -> EmailResponse:
        ...
        """
        response_data = self._make_request("POST", "/email/send", {'emails': [email.to_dict()]})
    ...
    def send_emails(self, emails: EmailList) -> EmailResponseList:
        ...
        response_data = self._make_request("POST", "/email/send", {"emails": email_data})
